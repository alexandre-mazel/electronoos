...
def playAllFileFromFolder( strPath, listToExclude=[] ):
...
if __name__ == "__main__":
    try:
        ...
        if len(sys.argv)>1:
            mp3file = sys.argv[1]
            playSongInterruptible(mp3file)
            # trouve des chansons et les joue
            strPath, strFilename = os.path.split(mp3file)
            playAllFileFromFolder(strPath+os.sep, [strFilename])

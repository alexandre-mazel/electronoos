...
def generateImage( nNumMark, strName, strDstPath = "./generated/", strSrcPath = "./imgs/" ):
    ...
    w_aru = int(w/3.4) # /3.1 define the final size of the mark on the page
    h_aru = w_aru
    
    xaru = (w-w_aru)//2
    rAruMarginCoef = 0.035 # 0.015
    ...
    cv2.putText(im, str(nNumMark), (w-nMarginBorder-20-100, int(h-nMarginBorder-20) ), cv2.FONT_HERSHEY_SIMPLEX, 1.4, black, 1)
    
    if 1:
        nThickness = nMarginBorder # 30=> 3mm of plastic border after cut
    ...
    if 1:
        ...
        im[h-hlf-nMarginBorder-nAddedMargin:h-nMarginBorder-nAddedMargin,nMarginBorder+nAddedMargin:nMarginBorder+nAddedMargin+wlf] = logo
...
# generateFromCsv - end    
generateFromCsv( "aruco_global_list.csv" )